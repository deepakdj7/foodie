...
const Layout = ({ location, title, image, children }) => {
  ...
  const onSearchToggle = (isSearch) => {
    if (state.isSearch !== isSearch) {
      setState({
        isSearch
      })
    }
  ...
  let imageSrc = null;
  result.allFile.edges.forEach(imageItem => {
  ...
  return (
    ...
    >
      ...
      <div className={"main main-content relative " + (state.isSearch ? 'search-active' : '')}>
        <div className="main-header">
          <Search posts={result.allMarkdownRemark.edges} show={() => onSearchToggle(true)} hide={() => onSearchToggle(false)} />
        </div>
        <main>{children}</main>
        <Footer />
        <div className="search-overlay" onClick={() => onSearchToggle(false)}></div>
      </div>
    </div>
  )
}
